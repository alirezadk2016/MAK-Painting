...
export function Footer() {
  ...
  return (
    <footer className="bg-charcoal text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 pb-24 lg:pb-8">
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          ...
          <div className="lg:col-span-1">
            ...
            <a href={MAPS_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 bg-white/10 hover:bg-white/15 rounded-xl px-3 py-2 mb-4 transition-colors">
              ...
              <span className="text-gray-400 text-xs">{t("reviews", { count: BRAND.googleReviewCount })}</span>
            </a>
          ...
          {/* Services */}
          <div>
            <h4 className="text-xs font-bold uppercase tracking-widest text-gray-500 mb-4">{t("servicesHeading")}</h4>
          ...
          {/* Company */}
          <div>
            <h4 className="text-xs font-bold uppercase tracking-widest text-gray-500 mb-4">{t("companyHeading")}</h4>
            <ul className="space-y-2.5">
              {[
                [tn("about"), "/about"],
                [tn("services"), "/services"],
                [tn("gallery"), "/gallery"],
                [tn("contact"), "/contact"],
              ].map(([label, href]) => (
                <li key={href}>
          ...
          {/* Service Areas */}
          <div>
            <h4 className="text-xs font-bold uppercase tracking-widest text-gray-500 mb-4">{t("areasHeading")}</h4>
            <div className="flex flex-wrap gap-1.5">
              ...
              <span className="text-xs text-gold/80">{t("allSuburbs")}</span>
            </div>
            <div className="mt-5 space-y-2">
              <a href={`tel:${BRAND.phone.replace(/\s/g,"")}`} dir="ltr" className="flex items-center gap-2 text-sm text-gray-400 hover:text-white transition-colors rtl:justify-end">
        ...
        <div className="border-t border-white/10 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-xs text-gray-600">
            {t("rights", { year, types: BRAND.serviceTypes, suburb: BRAND.suburb })}
          </p>
          <div className="flex items-center gap-4 text-xs text-gray-600">
            <a href={MAPS_URL} target="_blank" rel="noopener noreferrer" className="hover:text-gray-400 transition-colors">{t("googleMaps")}</a>
            <span>{t("gst")}</span>
          </div>
        </div>
      </div>
